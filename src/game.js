class Game {
    ...
    listenClick() {
        ...
        addEventListener('click', function (event) {
            ...
            var addingCoord = stone.getPointCoords(board, mousePos);

            if (addingCoord && !board.isStoneExists(addingCoord)) {
                painter.drawStone(addingCoord, stone, board, game);

                board.previousStone = addingCoord;

                //destination - an array where coords will be added
                var destination = board.stoneCoords[this.currentPlayerColor];

                var indexOfCluster = game.addCoords(board, addingCoord);
                game.checkClosure(board, indexOfCluster);
                game.switchPlayer();
...
//checks all sides of current stone
    checkSides(currentStone, suspiciousCoords, surroundedCoords, board, indexOfCluster, maxX, maxY) {
        var color = this.currentPlayerColor;
        ...
        for (var i = 0; i < board.stoneCoords[enemyColor].length; i++) {
            ...
            for (var j = 0; j < board.stoneCoords[enemyColor][i].length; j++) {
                ...
                if (surrounded.x === current.x && surrounded.y === current.y) {
                    ...
                    this.score[this.currentPlayerColor]++;
                    return;
                }
            }
        }
